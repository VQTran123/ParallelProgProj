...
int rank_merge(long *sendbuf, long *recvbuf, int count, MPI_Datatype datatype, MPI_Comm comm, int step) {
    int rank;
    ...
    if(rank % step == 0) {
        ...
        if(rank > pair) {
            ...
            sendbuf = recalloc(sendbuf, arr_size*2*sizeof(long));
            int i = arr_size;
            while(i < arr_size*2){
...
int main(int argc, char** argv) {
    ...
    while(step < size) {
        ...
        rank_merge(&array, &placeholder_array, 0, MPI_LONG, MPI_COMM_WORLD, step);
        step *= 2;
    }
