    ...
    if(rank % step == 0) {
        ...
        if(rank > pair) {
            ...
            *right_arr = *recvbuf
            ...
            long* curr_arr = calloc(arr_size*2, sizeof(long));
            int i = 0;
            while(i < arr_size){
                curr_arr[i] = sendbuf[i];
                curr_arr[i+arr_size] = right_arr[i];
            }
            merge(curr_arr, 0, arr_size, arr_size*2)
            sendbuf = calloc(arr_size*2, sizeof(long));
            *sendbuf = *curr_arr;
        }
    }
}
